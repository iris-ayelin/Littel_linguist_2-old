...
export class MixedLettersGameComponent implements OnInit {
  ...
  dialog: any;
  feedbackIncorrect: string | undefined;
  ...
  private getRandomWord(words: { origin: string, target: string }[]): { origin: string, target: string } | null {
  ...
  checkGuess(): void {
    const isCorrect = this.userGuess.trim().toLowerCase() === this.randomWord?.origin.toLowerCase();
    this.feedbackCorrect = isCorrect ? 'Correct! Guess the next word' : 'Try again!';
    this.feedbackIncorrect = isCorrect ? 'correct-feedback' : 'incorrect-feedback';
